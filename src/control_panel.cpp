...
void reply(Message &m, struct server &s, struct client &c, int error)
{
	...
	switch (error) {
		...
		case ERR_WASNOSUCHNICK  : send_error(s, c,":" + s._servername +" " + to_string(error) + " " + c._nickname + " " +  m.getContent()[nick] + " :There was no such nickname");break;
		...
		case ERR_NOPERMFORHOST  : send_error(s, c,":" + s._servername +" " + to_string(error) + " " + c._nickname + " " +  ":Your host isn't among the privileged");break;
		...
		case ERR_CHANNELISFULL  : send_error(s, c,":" + s._servername +" " + to_string(error) + " " + c._nickname + " " +  m.getContent()[chan] + ":Cannot join channel (+l)");break;
		case ERR_UNKNOWNMODE  : send_error(s, c,":" + s._servername +" " + to_string(error) + " " + c._nickname + " " +  m.getContent()[chan] + ":is unknown mode char to me");break;
		case ERR_INVITEONLYCHAN  : send_error(s, c,":" + s._servername +" " + to_string(error) + " " + c._nickname + " " +  m.getContent()[chan] + ":Cannot join channel (+i)");break;
		case ERR_BANNEDFROMCHAN  : send_error(s, c,":" + s._servername +" " + to_string(error) + " " + c._nickname + " " +  m.getContent()[chan] + ":Cannot join channel (+b)");break;
		case ERR_BADCHANNELKEY  : send_error(s, c,":" + s._servername +" " + to_string(error) + " " + c._nickname + " " +  m.getContent()[chan] + ":Cannot join channel (+k)");break;
