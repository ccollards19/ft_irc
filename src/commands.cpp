...
bool	isClientInChannel(channel &chan, std::string nickname) {
	for (int i = 0; i < chan._members.size(); i++) {
		if (chan._members[i].getNickname() == nickname)
			return (true);
	}
	return (false);
}
...
void	server::TOPIC(Message &m, client *client) {
	...
	if (params.at(1).empty())
		reply(ERR_NEEDMOREPARAMS);
	try {
		std::vector<Channel>::iterator chan = getChannel(params.at(1));
		if (isClientInChannel(*chan, client.getNickname()))
		{
			if (args.size() == 2)
			{
				if (chan->getTopic().empty()) //getTopic a coder
					reply(RPL_NOTOPIC);
				else
					reply(RPL_TOPIC);
			}
			...
			{
				chan->setTopic(getTopic()); //setTopic a coder
				reply(RPL_TOPIC);
			}
			else
			...
			cl.reply(ERR_NOTONCHANNEL());
	}
	catch {
		reply(ERR_NOSUCHCHANNEL) // verifier si necessaire
	}
}
...
void	server::PART(Message &m, client *client) {
	std::vector<std::string> params = m.getContent();
	if (params.size() < 2)
		reply(reply(m, *this, *client, ERR_NEEDMOREPARAMS));
	try {
		// std::vector<Channel>::iterator chan = getChannel(params.at(1));
		// Que faire si le createur leave le channel ?
		chan->eraseClient(client);
		if (chan->getClients().empty())
			_chan_list.erase(params.at(1));
	}
